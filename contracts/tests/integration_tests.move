...
module sonar::integration_tests {
    ...
    #[test]
    fun test_purchase_dataset_kiosk_with_sui_basic_flow() {
        let mut scenario = ts::begin(ADMIN);
        setup_marketplace(&mut scenario);

        // Submit audio as admin/uploader
        ts::next_tx(&mut scenario, ADMIN);
        {
            let mut marketplace = ts::take_shared<QualityMarketplace>(&scenario);
            let burn_fee = coin::mint_for_testing<SONAR_TOKEN>(1_000_000_000, ts::ctx(&mut scenario));

            marketplace::submit_audio(
                &mut marketplace,
                burn_fee,
                string::utf8(b"blob_kiosk"),
                string::utf8(b"seal_policy_kiosk"),
                option::none(),
                180,
                ts::ctx(&mut scenario)
            );
        ...
        // Finalize submission and list for sale
        ts::next_tx(&mut scenario, ADMIN);
        {
            let mut marketplace = ts::take_shared<QualityMarketplace>(&scenario);
            let validator_cap = ts::take_from_sender<ValidatorCap>(&scenario);
            ...
            marketplace::list_for_sale(&mut submission, 1_500_000_000, ts::ctx(&mut scenario));
        ...
        // Fund kiosk with SONAR reserve
        ts::next_tx(&mut scenario, ADMIN);
        {
            let mut marketplace = ts::take_shared<QualityMarketplace>(&scenario);
            let admin_cap = ts::take_from_sender<AdminCap>(&scenario);
            let sonar_funding = coin::mint_for_testing<SONAR_TOKEN>(5_000_000_000, ts::ctx(&mut scenario));

            marketplace::fund_kiosk_sonar(
                &admin_cap,
                &mut marketplace,
                sonar_funding
            );
        ...
        // Purchase dataset via kiosk using SUI payment (transaction executed by admin for test simplicity)
        ts::next_tx(&mut scenario, ADMIN);
        {
            let mut marketplace = ts::take_shared<QualityMarketplace>(&scenario);
            let mut submission = ts::take_from_sender<AudioSubmission>(&scenario);
            let sui_payment = coin::mint_for_testing<SUI>(2_000_000_000, ts::ctx(&mut scenario));

            marketplace::purchase_dataset_kiosk_with_sui(
                &mut marketplace,
                &mut submission,
                sui_payment,
                ts::ctx(&mut scenario)
        ...
        // Validate marketplace stats updated
        ts::next_tx(&mut scenario, ADMIN);
        {
            let marketplace = ts::take_shared<QualityMarketplace>(&scenario);
            let (_total_submissions, total_purchases, _total_burned, _, _) =
                marketplace::get_marketplace_stats(&marketplace);
            assert!(total_purchases == 1, 1);
            ts::return_shared(marketplace);
        };
