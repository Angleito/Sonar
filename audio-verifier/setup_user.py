...
Creates /etc/passwd and /etc/group files and adds the verifier user.
"""
import os
import stat
...
def main():
    ...
    verifier_uid = 1000
    verifier_gid = 0  # Use root group (GID 0)
    verifier_home = Path('/home/verifier')
    
    # Add verifier user to passwd
    add_user_entry(passwd_file, 'verifier', verifier_uid, verifier_gid, 
                   str(verifier_home), '/bin/sh')
    
    # Add verifier group to group file
    add_group_entry(group_file, 'verifier', verifier_gid)
    
    # Create home directory
    verifier_home.mkdir(parents=True, exist_ok=True)
    
    # Set ownership and permissions
    os.chown(verifier_home, verifier_uid, verifier_gid)
    os.chmod(verifier_home, 0o755)
    
    # Set ownership for /app and /tmp/audio-verifier
    app_dir = Path('/app')
    tmp_dir = Path('/tmp/audio-verifier')
    ...
    if app_dir.exists():
        chown_recursive(app_dir, verifier_uid, verifier_gid)
        os.chmod(app_dir, 0o755)
    
    if tmp_dir.exists():
        chown_recursive(tmp_dir, verifier_uid, verifier_gid)
        os.chmod(tmp_dir, 0o1777)  # Sticky bit for temp directory
